...
(function() {
  var attached_sign_mode, compute_mac_key, createHash, crypto, crypto_onetimeauth_BYTES, crypto_secretkey_BYTES, current_major, current_minor, detached_sign_mode, encryption_mode, msgpack, nonce, prng, util;
  ...
  exports.generate_encryption_header_packet = function(encryptor, recipients) {
    var crypto_hash, ephemeral_encryptor, header_hash, header_intermediate, header_list, mac_keys, payload_encryptor, payload_key, rec_pair, rec_payload, rec_pubkey, recipients_list, sender_sbox, _i, _j, _len, _len1;
    header_list = [];
  ...
  exports.parse_encryption_header_packet = function(decryptor, header_intermediate) {
    var auth_index, crypto_hash, ephemeral, error, format, found, header_hash, header_list, mac_key, major, minor, mode, payload_decryptor, payload_key, recipients, secret, sender, sender_pubkey, _i, _j, _ref, _ref1, _ref2;
    crypto_hash = createHash('sha512');
    crypto_hash.update(header_intermediate);
    header_hash = crypto_hash.digest();
    ...
    for (auth_index = _i = 0, _ref1 = recipients.length; 0 <= _ref1 ? _i < _ref1 : _i > _ref1; auth_index = 0 <= _ref1 ? ++_i : --_i) {
      if (util.bufeq_secure(recipients[auth_index][0], decryptor.publicKey)) {
        payload_key = decryptor.box_open_afternm({
          ciphertext: recipients[auth_index][1],
          nonce: nonce.nonceForPayloadKeyBox(),
          secret: secret
        });
        found = true;
    ...
    if (!found) {
      for (auth_index = _j = 0, _ref2 = recipients.length; 0 <= _ref2 ? _j < _ref2 : _j > _ref2; auth_index = 0 <= _ref2 ? ++_j : --_j) {
        try {
          payload_key = decryptor.box_open_afternm({
            ciphertext: recipients[auth_index],
            nonce: nonce.nonceForPayloadKeyBox(),
            secret: secret
          });
        } catch (_error) {
