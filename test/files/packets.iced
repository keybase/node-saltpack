...
gen_header = (opts) ->
  ...
  packed_header = header.generate_encryption_header_packet(alice, recipients_list, {anonymized_recipients})
  return {packed_header, alice, bob}

_test_header_pipeline = (T, opts) ->
  {packed_header, _, bob} = gen_header(opts)
  {mac_key, recipient_index} = header.parse_encryption_header_packet(bob, packed_header.header_intermediate)
...
exports.test_anonymous_sender = (T, cb) ->
  ...
  {_, _, _, sender_pubkey, _, _} = header.parse_encryption_header_packet(bob, packed_header.header_intermediate)
...
exports.test_payload_pipeline = (T, cb) ->
  ...
  {header_list, header_hash, payload_key, sender_pubkey, mac_key, recipient_index} = header.parse_encryption_header_packet(bob, packed_header.header_intermediate)
