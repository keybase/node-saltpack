...
generate_anonymous_header = (alice, pk) ->
  ...
  header.generate_encryption_header_packet(alice, recipients, {anonymized_recipients})
...
exports.break_mac_key_in_payload_packet = (T, cb) ->
  ...
  {recipient_index} = header.parse_encryption_header_packet(bob, header_intermediate)
  ...
  # break the MAC key
  {_, _, _, _, mac_key, _} = header.parse_encryption_header_packet(bob, header_intermediate)
  mac_key[0] = ~mac_key[0]
